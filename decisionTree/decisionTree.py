...
def computation(jobId,featurePath,targetPath,testingPath):
    ...
    log("*" * 30 + " feature importance " + "*" * 30)
    print(dec_tree.feature_importances_)
    log(f"{dec_tree.feature_importances_}")
    ...
    with open(folder+'/model.pickle', 'wb') as handle:
        pickle.dump(dec_tree, handle)

    modelBytes = pickle.dumps(dec_tree)
