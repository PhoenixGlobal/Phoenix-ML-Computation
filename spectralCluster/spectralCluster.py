...
def computation(jobId,featurePath):
    ...
    tradingDataComplexity = tradingDataSp[0] * tradingDataSp[1] * 110
    ...
    for index, gamma in enumerate((0.01, 0.1, 1, 10)):
        for index, k in enumerate((3, 4, 5, 6)):
            y_pred = SpectralClustering(n_clusters=k, gamma=gamma).fit_predict(x_train)
            ...
            log(f"Calinski-Harabasz Score with gamma={gamma},n_clusters={k},score:{score}")

    print('best params are ',bestK,bestGamma,maxScore)
    log(f"best params are {bestK},{bestGamma},{maxScore}")

    spectralClusterModel = SpectralClustering(n_clusters=bestK, gamma=bestGamma).fit(x_train)
    ...
    with open(folder+'/model.pickle', 'wb') as handle:
        pickle.dump(spectralClusterModel, handle) #, protocol=pickle.HIGHEST_PROTOCOL

    modelBytes = pickle.dumps(spectralClusterModel)
    ...
    out = SpectralClustering(n_clusters=bestK, gamma=bestGamma).fit_predict(x_test2)
